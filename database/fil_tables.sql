...
--
-- Очищення таблиць перед заповненням (опціонально)
--
...
(1, 'Головний Адмін', 'admin@example.com', '$2y$10$fW.1.Yups8QPkZ3jOE6vTu3S1ajw2g5S8V8C55qhbZxdgBv0uMh.S', 1),
...
(4, 'Одяг', 1, NULL),
(5, 'Футболки', 1, 4),
(6, 'Джинси', 0, 4), -- Неактивна категорія
(7, 'Книги', 1, NULL);

--
-- Наповнення таблиці `goods`
--
INSERT INTO `goods` (`id`, `name`, `description`, `price`, `weight`, `category_id`, `is_active`) VALUES
...
--
INSERT INTO `photo_albums` (`id`, `name`, `description`, `parent_id`, `cover_image_id`) VALUES
...
INSERT INTO `photos` (`id`, `album_id`, `filename`, `note`) VALUES
...
-- Оновлюємо обкладинки альбомів
UPDATE `photo_albums` SET `cover_image_id` = 1 WHERE `id` = 2;
UPDATE `photo_albums` SET `cover_image_id` = 3 WHERE `id` = 3;

--
-- Імітація руху товарів: `product_transactions` та `product_stock`
--

-- 1. Перше велике надходження на Основний склад (Київ), робить Адміністратор (id=1)
INSERT INTO `product_transactions` (`transaction_date`, `good_id`, `warehouse_id`, `quantity`, `balance`, `transaction_type`, `document_type`, `user_id`) VALUES
...
-- Оновлюємо актуальні залишки в `product_stock`
...
-- 2. Переміщення частини товарів до Львова, робить Менеджер (id=2)
-- Смартфони:
INSERT INTO `product_transactions` (`transaction_date`, `good_id`, `warehouse_id`, `quantity`, `balance`, `transaction_type`, `document_type`, `user_id`, `related_transaction_id`, `comment`) VALUES
...
-- Оновлюємо актуальні залишки в `product_stock` після переміщення
UPDATE `product_stock` SET `quantity` = 80.000 WHERE `good_id` = 1 AND `warehouse_id` = 1;
INSERT INTO `product_stock` (`good_id`, `warehouse_id`, `quantity`) VALUES (1, 2, 20.000);
UPDATE `product_stock` SET `quantity` = 40.000 WHERE `good_id` = 2 AND `warehouse_id` = 1;
INSERT INTO `product_stock` (`good_id`, `warehouse_id`, `quantity`) VALUES (2, 2, 10.000);

--
-- Наповнення таблиць чату
--
INSERT INTO `chat_groups` (`id`, `group_name`) VALUES (1, 'Загальний чат');
...
INSERT INTO `chat_messages` (`sender_id`, `recipient_id`, `group_id`, `body`, `created_at`) VALUES
-- Приватний діалог між Адміном (1) та Менеджером (2)
