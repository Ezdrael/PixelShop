<?php
// app/views/order/track_detail.php
...
<section class="breadcrumbs">
    <div class="container">
        ...
        <span>/</span>
        <a href="<?php echo BASE_URL; ?>/order/track"><i class="fa-solid fa-clipboard-check me-1"></i>Мої Замовлення</a>
        <span>/</span>
        <span><i class="fa-solid fa-clipboard-check me-1"></i>Деталі Замовлення</span>
    </div>
</section>
                ...
                <?php elseif ($order): ?>
                    <div class="card shadow-sm p-4 animate__animated animate__fadeIn">
                        <div class="text-center mb-4">
                            ...
                            <p class="lead text-muted animate__animated animate__fadeInUp animate__delay-0-5s">Дякуємо за ваше замовлення, <?php echo htmlspecialchars($order['customer_name'] ?? ''); ?>!</p>
                        </div>

                        <div class="mb-4 animate__animated animate__fadeInUp animate__delay-1s">
                            <h2 class="h4 fw-bold mb-3"><i class="fa-solid fa-clipboard-check me-2"></i>Деталі замовлення #<?php echo htmlspecialchars($order['tracking_id'] ?? ''); ?></h2>
                            <ul class="list-group list-group-flush">
                                ...
                                <li class="list-group-item d-flex justify-content-between align-items-center">
                                    Загальна сума:
                                    <span class="fw-bold text-primary"><?php echo htmlspecialchars($order['displayTotalAmount'] ?? '0 грн'); ?></span>
                                </li>
                                <li class="list-group-item">
                                    Дата замовлення:
                                    <span class="float-end"><?php echo htmlspecialchars(date('d.m.Y H:i', strtotime($order['created_at'] ?? 'now'))); ?></span>
                                </li>
                                <li class="list-group-item">
                                    Email:
                                    <span class="float-end"><?php echo htmlspecialchars($order['customer_email'] ?? ''); ?></span>
                                </li>
                                <li class="list-group-item">
                                    Телефон:
                                    <span class="float-end"><?php echo htmlspecialchars($order['customer_phone'] ?? ''); ?></span>
                                </li>
                                <li class="list-group-item">
                                    Адреса доставки:
                                    <span class="float-end"><?php echo htmlspecialchars($order['delivery_address'] ?? ''); ?></span>
                                </li>
                                ...
                                <?php if (!empty($order['items'])): ?>
                                    <?php foreach ($order['items'] as $item): ?>
                                        <li class="list-group-item d-flex justify-content-between align-items-center">
                                            ...
                                            <span class="badge bg-secondary rounded-pill"><?php echo htmlspecialchars($item['quantity'] ?? '0'); ?> шт. x <?php echo htmlspecialchars($item['displayPrice'] ?? '0 грн'); ?></span>
                                        </li>
                                    <?php endforeach; ?>
                                <?php else: ?>
                                    <li class="list-group-item text-center text-muted">Немає товарів у цьому замовленні.</li>
                                <?php endif; ?>
                            </ul>
                        </div>
                        ...
                        <div class="text-center mt-4 animate__animated animate__fadeInUp animate__delay-2s">
                            ...
                            <a href="<?php echo BASE_URL; ?>/order/track/<?php echo htmlspecialchars($order['tracking_id'] ?? ''); ?>" class="btn btn-outline-primary btn-lg"><i class="fas fa-link me-2"></i>Відстежити Замовлення</a>
