    ...
    <form action="" method="POST" id="event-form">
        ...
        <div class="form-body">
            ...
            <div class="form-group-inline">
                ...
                <input type="datetime-local" id="event-start" name="start_time" class="form-control" value="<?php echo !empty($event['start_time']) ? date('Y-m-d\TH:i', strtotime($event['start_time'])) : ''; ?>" required>
            </div>
            <div class="form-group-inline">
                <label for="event-end">Час закінчення</label>
                <input type="datetime-local" id="event-end" name="end_time" class="form-control" value="<?php echo !empty($event['end_time']) ? date('Y-m-d\TH:i', strtotime($event['end_time'])) : ''; ?>">
            </div>
            <div class="form-group-inline">
                <label for="event-description">Опис</label>
                <textarea id="event-description" name="description" class="form-control" rows="5"><?php echo htmlspecialchars($event['description'] ?? ''); ?></textarea>
            </div>
        </div>
    </form>
</div>
